...
println!("--------------get_protocol_features and node.protocol_features = {:x}-----", node.protocol_features);
...
println!("--------------set_protocol_features and node.acked_protocol_features = {:x}-----", node.acked_protocol_features);
...
println!("--------------get_config and node.acked_protocol_features = {:x}-----", node.acked_protocol_features);
            return error_code(VhostUserError::InvalidOperation);
        }

println!("------1--------get_config ---------");
...
impl MasterInternal {
    ...
    fn recv_reply_with_payload<T: Sized + Default + VhostUserMsgValidator>(
            ...
            || !body.is_valid()
        {
            ...
            println!("-----------2222222222-----bytes={:x}--- and buf.len={:x}", bytes, buf.len() );
