...
impl Blk {
    ...
    pub fn new(path: &str, num_queues: usize, queue_size: u16, config_wce: u8) -> Result<Blk> {
        ...
        println!("-------0------blk::new() avail_features = 0x{:x}-----", avail_features);
...
println!("-------1-----setup_vub() acked_features = 0x{:x}-----", self.acked_features);
...
impl VirtioDevice for Blk {
    ...
    fn activate(
        ...
        let vub_interrupt_list = self.setup_vub(mem, &queues, queue_evts).unwrap();
