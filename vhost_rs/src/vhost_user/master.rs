...
impl MasterInternal {
    ...
    fn send_request_with_config_body(
        ...
        let hdr = Self::new_request_header(code, mem::size_of::<VhostUserConfig>() as u32  + user_config.size);
        let mut buf = vec![0; user_config.size as usize];
        self.main_sock.send_config_message(&hdr, user_config, &mut buf, fds)?;
        Ok(hdr)
    }
