...
pub const VHOST_USER_F_PROTOCOL_FEATURES: ::std::os::raw::c_uint = 30;
...
impl Net {
    ...
    pub fn new(mac_addr: MacAddr, path: &str, num_queues: usize, queue_size: u16) -> Result<Net> {
        ...
        // The following features action is to fill features in master.node,
        // in order to get queue number.
        ...
        let max_queue_number = if backend_features & (1 << VHOST_USER_F_PROTOCOL_FEATURES) != 0 {
            ...
            vhost_user_net.get_queue_num().unwrap()
        } else {
            num_queues as u64
        };
...
impl VirtioDevice for Net {
    ...
    fn activate(
        ...
        println!("Entering vhost-user net activate ......!");
        if queues.len() > self.max_queue_number as usize {
            error!(
                "Cannot perform activate. {} queue(s) is larger than the maximum queue number {} slave supports",
                queues.len(),
